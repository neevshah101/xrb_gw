...
Gravitational wave (GW) observations have revealed certain challenging to interpret “outlier” events. This includes GW190814, a highly asymmetric ($q\approx 0.1$) merger involving a $23\,\Mdot$ black hole merging with a $2.6\,\Mdot$ compact object in the hypothesized “lower-mass-gap” between neutron stars (NS) and black holes (BH). Such highly asymmetric mass-ratio systems are difficult to create through most formation scenarios, and the presence of a compact object in the lower-mass-gap as well makes GW190814 a particularly interesting system to study. We demonstrate how local electromagnetic observations of massive binaries can be utilized to understand rare outcomes such as GWs, particularly the “outliers” among them. To understand GW190814, we leverage local observations of an analogous runaway high-mass X-ray binary (HMXB), 4U 1700-37, in our own Galaxy, consisting of a $\approx 2\,\Mdot$ NS/ low-mass BH and a $40-50\,\Mdot$ companion. Constrained by observables such as its orbital properties, kinematics, and age, we use monte-carlo simulations to infer the pre-supernova parameters. Using them, we compute detailed binary evolution models with \textsc{MESA}, and reconstruct the evolutionary history of the HMXB. We find that its observed properties can only be reproduced by starting from moderately symmetric mass-ratio (q>0.7) massive binaries in a short orbit, and close-to-conservative mass transfer. Additionally, we find that the $\approx 2\,\Mdot$ compact object likely received a directed natal kick to shrink the orbital period post-supernova. We show that this system is unlikely to form a GW source due to a failed Common Envelope (CE) phase in the future. With similar models computed at lower metallicity, we find that GW190814-like events naturally arise from close-to-symmetric mass-ratio (q>0.9) binaries, provided mass transfer is close-to-conservative, and the NS/ low-mass BH receives a strong natal kick that widens the orbit post first-supernova. This allows the subsequent interaction to occur when the companion star develops a loosely bound convective-envelope that is easier to eject during the CE phase. This study highlights the importance of the interplay between finding, and characterizing, rare and common products of massive binaries with various surveys, understanding their formation with detailed modeling, and connecting them to distant GW events to constrain uncertainties in massive stars and their compact remnants.
...
(ZAMS), but this can change due to tidal effects or binary interactions \mr{[tend to or do they? Say what
happens in your models (possibly quoting the timescale for sync)]} \red{rephrased}. We include several
...
\citep{1997A&A...327..620S,2017MNRAS.471.4256V}. \mr{[are we going to
discuss parameter variations of these? If so, maybe announce in Sec. X
(or Appendix Y) we discuss variations of these parameters]} \red{barely discuss it in a discussion section, without showing any plots. so i think its fine to leave this as fiducial setup} We also
...
\red{Based on the results of our monte carlo simulations in Section \ref{sec:monte-carlo-results}, we compute binary evolution models that can reproduce 1) the pre-SN mass of the progenitor of the compact object 2) the estimated mass of the companion star in the HMXB today and 3) the pre--SN orbital period of the binary.} To illustrate the evolution and structure of our binary models,
...
in a tight orbit with a period of $3\,\mathrm{days}$ \mr{[these
...
could make sense]} \red{fair point but i think it is more a of a distract than a benefit to the reader and the broad goal of the paper. I added some text in the beginning in red that mentions the three criteria I use for evaluating the "success" of a model}. In Fig. \ref{fig:xrb_fiducial_hr}, we show the evolution of the donor and accretor on the HR diagram. Due to the short initial period, the donor overflows its roche lobe after $3.26\,\mathrm{Myr}$, during the Main Sequence (MS) itself. This leads to a phase of mass transfer from the donor to the accretor, referred to as Case A \citep{1967ZA.....65..251K}. This consists of an initial thermal timescale mass transfer ($\sim 0.07\, \mathrm{Myr}$), known as fast Case A (highlighted in blue in Fig. \ref{fig:xrb_fiducial_hr})., %where the donor loses a majority of the mass lost during RLOF. 
...
Once the companion star in the HMXB 4U 1700-37 fills its Roche Lobe, it will start transferring mass onto the compact object. However, given the highly asymmetric mass ratio of the system ($q \approx 1/20$), this is likely to become unstable. \red{We find that the mass transfer rate exceeds $0.1 \, \Mdot/\mathrm{yr}$, which generally signifies the onset of dynamical instability \citep{2025arXiv251009745K}. We assume such systems to undergo a Common Envelope phase \citep[e.g.,][]{Fragos23_posydon}. However, we discuss an alternative scenario in Section \ref{stable}.} 
...
shortened -- it's not a review on CE!} \red{I dont think I am reviewing CE, most of the text is just defining the symbols, for which I also need to define the equations}
...
such systems are not GW sources, they could appear as energetic transients such as LFBOTs \citep{2025arXiv251009745K} (see \cite{2025Ap&SS.370...11G} for a review on other possible multi-messenger signatures) or form a Thorne-\.Zytkow Object \citep[e.g,][]{thorne:75, TZO_review}.
...
In Fig. \ref{fig:gw_hr_accretor}, we show the evolutionary track of the accretor in the binary with blue highlighting Case MT, and red denotes Case AB MT.
...
receive a natal kick at birth, in analogy with the case of the HMXB 4U 1700-37. Depending on its strength and direction, a variety of post--SN outcomes are possible. This is unlike the Galactic HMXB 4U 1700-37, for which we had observational constraints on the post--SN orbit.
\begin{figure}[htbp]
    \centering
...
footnote, maybe move to methods}\red{I removed the footnote and mentioned it on line 681-683. This was not in methods, as the methods section was part of the evolution of the XRB, where this was not needed. I think its more appropriate to keep this here.}

\red{To account for the different possible outcomes post--SN, we detach the companion star from the compact object, and follow its evolution as a single star in \textsc{MESA}. This allows us to model the evolution of its density structure and envelope binding energy along with its radial evolution, thereby accounting for different possible post--SN orbital separations.}
...
lognormal distribution with $\mu = 5.60$ and $\sigma = 0.68$ \citep{2025ApJ...989L...8D}. The yellow shaded band corresponds to post--SN Roche Lobes that are smaller than $1250\,\Rdot$, which would only appear as a highly asymmetric mass-ratio HMXB, but not form a GW source due to a failed CE phase. The purple shaded band corresponds to post--SN Roche Lobes that are $\approx 1250-1500\, \Rdot$, which will likely have a successful CE phase and form a highly asymmetric mass-ratio GW source. The blue shaded band corresponds to orbits that are so large that the star would never fill its Roche Lobe. For the assumed kick distribution, we find that only $\approx 0.24\%$ of systems that may appear as a HMXB can form a GW source through a successful CE phase. Thus we can estimate the local rate of highly asymmetric GW190814--like events to be
 ...
 by the LVK collaboration \citep{2020ApJ...896L..44A}, which is based on a single event (GW190814). However, we caution that our rate estimate depends on key uncertain factors such as the lifetime of the HMXB phase, and the radial extent of the star for a successful CE phase.%, and is a poisson rate.
 A detailed rate calculation also requires the use of
...
should aim at compressing} \red{dont really know what to compress or remove}
...
asymmetric Galactic HMXBs and GW sources. In both scenarios, there are
...
of the binary to invert and the accretor to become much more massive than the donor. Secondly, we also require that the the donor
star successfully explodes at the end of its life, and leaves behind a
low mass compact object which could either be a NS or a low mass BH. \red{Observations of the Galactic HMXB 4U 1700-37 strongly suggest that this pathway can occur in nature, as this system consists of a low mass compact object with a massive stellar companion.}
