...
With the help of observational constraints on the parameters of a post--SN binary, we can infer its pre--SN parameters. We follow the setup in \citep{2024OJAp....7E..38E}, which in turn uses \citep{1994astro.ph.12023B} for calculating the outcomes of a SN in a binary. These calculations implicitly assume that the mass lost in a SN explosion is instantaneously lost from the system, which is reasonable given the very high ejecta speeds in a SN in comparison to the orbital velocity.
...
Based on the results of our monte carlo simulations in Section \ref{sec:monte-carlo-results}, we compute binary evolution models that can reproduce 1) the pre-SN mass of the progenitor of the compact object 2) the estimated mass of the companion star in the HMXB today and 3) the pre--SN orbital period of the binary. To illustrate the evolution and structure of our binary models,
...
At the end of the donor star's life, it may successfully explode and leave behind a $2 \, \Mdot$ compact object, and once the accretor grows large enough that it is close to filling its Roche Lobe, the system would resemble the HMXB 4U 1700-37 that we observe today. Due to previous mass accetion, we find that the surface of the companion star would appear to be enriched in helium and CNO--processed nitrogen, with surface mass fractions of $\approx 0.42$ and $\approx 0.004$ respectively (\red{write ZAMS multiplier}). In Sections \ref{sec:discussion}, we discuss certain limitations of our fiducial model, and provide further justification for why this system likely underwent Case A MT in the past.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.47\textwidth]{xrb_fiducial_hr.pdf}
    \caption{The evolution of the donor (top panel) and accretor star (bottom panel)on the HR diagram, in our fiducial model for forming the HMXB 4U 1700-37/ HD 153919 at $Z = Z_\odot = 0.0146$ \citep{2009ARA&A..47..481A}. The initial masses of the donor and accretor are $40\,\Mdot$ and $28\,\Mdot$, and the initial orbital period is $3\,\mathrm{days}$. The blue  and red curves highlight fast and slow Case A mass transfer respectively, while the green curve is for Case AB. The diamond, plus, and star markers correspond to the TAMS, core--helium ignition, and core--helium depletion respectively. The purple and blue markers with errorbars show the observed location of HD 153919 from x and y respectively. The inset panel zooms into the evolution of the donor during Case A mass transfer.}
    \label{fig:xrb_fiducial_hr}
\end{figure}

\section{Can the HMXB 4U 1700-37 form a GW source in the future?} \label{xrb-future}

Once the companion star HD 153919 in the HMXB fills its Roche Lobe\footnote{The X--rays observed today are likely from wind accretion, as expected in most HMXBs.}, it will start transferring mass onto the compact object. However, given the highly asymmetric mass ratio of the system ($q \approx 1/20$), this is likely to become unstable. We find that the mass transfer rate exceeds $0.1 \, \Mdot/\mathrm{yr}$, which generally signifies the onset of dynamical instability \citep{2025arXiv251009745K}. We assume such systems to undergo a Common Envelope (CE) phase \citep{Fragos23_posydon}. For an alternative scenario see Section \ref{stable}. 
...
such systems are not GW sources, they could appear as energetic transients such as LFBOTs \citep{2025arXiv251009745K} (see \cite{2025Ap&SS.370...11G} for a review on other possible multi-messenger signatures) or form a Thorne-\.Zytkow Object \citep[e.g,][]{thorne:75, 2025A&A...694A..83N,TZO_review}.

\section{Formation of GW190814} \label{sec:gw190814}

In the previous section, we constructed detailed binary evolution
models at solar metallicity to demonstrate the past evolutionary history of the most asymmetric HMXB in the Galaxy. However, we also showed that it is unlikely to lead to the formation of a highly asymmetric GW source, as the system is likely to merge in a future CE phase due to the star's high envelope binding energy. This motivates us to compute another set of detailed binary evolution models, and explore scenarios in which a CE phase can be successful, and lead to the formation of a highly asymmetric GW source such as GW190814.

The progenitor binaries of the GW events detected by the LVK formed when the universe was much younger, and therefore likely metal--poor. \red{\cite{2025ApJ...979..209V} find that BNS formation is not metallicity dependent, while BBH formation is meallicity dependent and more efficient at lower metallicity. 
...
In Fig. \ref{fig:gw_hr_accretor}, we show the evolutionary track of the accretor in the binary on the HR diagram, with blue highlighting Case A MT, and red denoting Case AB MT.
...
$2.6\,\Mdot$ secondary in GW190814. We discuss ``explodability'' in more detail in Sec.~\ref{sec:explodability}. The compact object may also
...
\begin{figure}[htbp]
    ...
    \caption{The evolution of the accretor star on the HR diagram, in our fiducial model for forming a GW190814--like binary at a $Z = Z_\odot/10 = 0.00146$. The initial masses of the donor and accretor are $27\,\Mdot$ and $25\,\Mdot$, and the initial orbital period is $3.5\,\mathrm{days}$. The blue curve highlights Case A mass transfer, while the red curve is for Case AB. The diamond, plus, and star markers correspond to the TAMS, core--helium ignition, and core--helium depletion respectively. The inset panel zooms into the late stages of the core--helium burning phase, where it ascends the RSG branch, and developes a convective envelope (shown in purple)}
...
lognormal distribution with $\mu = 5.60$ and $\sigma = 0.68$ \citep{2025ApJ...989L...8D}. The yellow shaded band corresponds to post--SN Roche Lobes that are smaller than $1250\,\Rdot$, which would only appear as a highly asymmetric mass-ratio HMXB, but not form a GW source due to a failed CE phase. The purple shaded band corresponds to post--SN Roche Lobes that are $\approx 1250-1500\, \Rdot$, which will likely have a successful CE phase and form a highly asymmetric mass-ratio GW source. The blue shaded band corresponds to orbits that are so large that the star would never fill its Roche Lobe. For the assumed kick distribution, we find that only $\approx 0.24\%$ of systems that may appear as a HMXB can form a GW source through a successful CE phase. Thus we can estimate the local rate of highly asymmetric GW190814--like events to be
 ...
 This back--of--the--envelope estimate is within an order of magnitude of the
...
source like GW190814. The blue shaded band denotes orbits that are large enough such that the star does not fill its Roche Lobe during its subsequent evolution.}
...
Below we discuss certain assumptions and caveats that are relevant to this work.
...
% \red{I have changed this to only include works that specifically compare to my scenario such as Zevin and Mandel. I dont think its appropriate to discuss Lu, Arca Sedda, and Katie's work here, as they do completely different things, and have been mentioned in the intro}
...
\begin{itemize}
    ...
    \item Utilizing the fact that there exist two highly asymmetric Galactic HMXBs, we estimate the local rate of highly asymmetric GW190814--like events to be $\approx 0.5\,\mathrm{Gpc}^{-3}yr^{-1}$, which is in excellent agreement with the observed rate of such events.
\end{itemize}
