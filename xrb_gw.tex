...
Gravitational wave (GW) observations have revealed certain challenging to interpret “outlier” events. This includes GW190814, a highly asymmetric ($q\approx 0.1$) merger involving a $23\,\Mdot$ black hole merging with a $2.6\,\Mdot$ compact object. % in the hypothesized “lower-mass-gap” between neutron stars (NS) and black holes (BH).
 Such highly asymmetric mass-ratio systems are difficult to create through most formation scenarios. %, and the presence of a compact object in the lower-mass-gap as well makes GW190814 a particularly interesting system to study. 
 We demonstrate how local electromagnetic observations of intermediate stages of massive binary evolution can be utilized to understand rare outcomes such as GWs, particularly the “outliers” among them. To understand the formation pathway of GW190814, we leverage local observations of an analogous runaway high-mass X-ray binary (HMXB) 4U 1700-37, in our own Galaxy, consisting of a $\approx 2\,\Mdot$ compact object and a $40-50\,\Mdot$ supergiant companion. Constrained by observables such as its orbital properties, kinematics, and age, we use monte-carlo simulations to infer the pre-supernova parameters. Using them, we compute detailed binary evolution models with \textsc{MESA}, and reconstruct the evolutionary history of the HMXB. We find that its observed properties can only be reproduced by starting from moderately symmetric mass-ratio massive binaries in a short orbit, and close-to-conservative mass transfer. Additionally, we find that the $\approx 2\,\Mdot$ compact object likely received a directed natal kick to shrink the orbital period post-supernova. We show that this system is unlikely to form a GW source due to a failed Common Envelope (CE) phase in the future. We compute models at lower metallicity to show that a similar pathway can naturally form GW190814-like events, provided mass transfer is close-to-conservative, and the first born NS/ low-mass BH receives a strong natal kick that widens the orbit post-supernova. This allows the subsequent interaction to occur when the companion star develops a loosely bound convective-envelope that is easier to eject during the subsequent CE phase, and eventually form a highly asymmetric mass--ratio compact binary like GW190814. This study highlights the importance of the interplay between finding, and characterizing, rare and common products of massive binary evolution with various surveys in the local universe, and using them as a bridge to understand the formation of GW sources.
...
\citep[][\texttt{r24.08.01}]{2011ApJS..192....3P,2013ApJS..208....4P,2015ApJS..220...15P,2018ApJS..234...34P,2019ApJS..243...10P,2023ApJS..265...15J}. Our inlists and choice of input parameters are public (\red{link zenodo/github}) and we summarize relevant parameters and assumptions below.
...
in a tight orbit with a period of $3\,\mathrm{days}$. In Fig. \ref{fig:xrb_fiducial_hr}, we show the evolution of the donor and accretor on the HR diagram. Due to the short initial period, the donor overflows its roche lobe after $3.26\,\mathrm{Myr}$, during the Main Sequence (MS) itself. This leads to a phase of mass transfer from the donor to the accretor, referred to as Case A \citep{1967ZA.....65..251K}. This consists of an initial thermal timescale mass transfer ($\sim 0.07\, \mathrm{Myr}$), known as fast Case A (highlighted in blue in Fig. \ref{fig:xrb_fiducial_hr})., %where the donor loses a majority of the mass lost during RLOF. 
...
such systems are not GW sources, they could appear as energetic transients such as LFBOTs \citep{2025arXiv251009745K} (see \cite{2025Ap&SS.370...11G} for a review on other possible multi-messenger signatures) or form a Thorne-\.Zytkow Object (T\.ZO) \citep[e.g,][]{thorne:75, TZO_review}.
...
previous section \ref{sec:mesa-xrb-results}, we found that the progenitor of the compact object lost a significant amount of mass during its evolution, and particularly during its cHeB phase. This was crucial in lowering its pre--SN mass down to what we inferred from our monte carlo simulations, and also potentially aid in its explodability.
...
Since we stop evolving our donor models at core helium depletion, we neglect the impact of binary and tidal interactions after this point. We assume that in some cases, the donor star successfully explodes at
...
($\alpha_{\rm th}=1$ in Eq.~\ref{eq:BE_def}), while the golden
color does not include the internal energy ($\alpha_{\rm th}=0$), and corresponds to the gravitational binding energy of the envelope. The solid lines corresponds to the $X = 0.1$ limit for defining the core--envelope boundary as described in Section \ref{xrb-future}, while the shaded bands encompass the $X = 0.01$ and $X = 0.3$ limits for defining the boundary. For most of its evolution, as the star grows in radius, there is a moderate decrease in its binding energy. However, during its cHeB phase, when it has grown to a size of almost $1250\,\Rdot$, it developes a loosely bound convective envelope and starts ascending the Red Super Giant (RSG) branch (highlighted in purple in Fig. \ref{fig:gw_hr_accretor}). This dramatically decreases the effective binding energy of its envelope, which may make it easier to eject during a potential CE phase \citep{2021A&A...645A..54K}.
...
\begin{itemize}
    \item GW190814, the most asymmetric mass ratio GW event till date has been challenging to understand through most formation scenarios.
    \item Leveraging local observations of intermediate stages of massive binary evolution can help understand GW events.
    \item With monte carlo simulations and detailed binary evolution models, we show that the HMXB 4U 1700-37 is a potential analog of GW190814.
    \item For the HMXB, we find that Case A MT and a natal kick are necessary to explain its current orbital properties. We also show that in the future, this system will likely undergo a failed CE phase.
    \item We show that a similar formation pathway works at lower metallicity, which is more relevant for GW sources. Conservative MT is essential to significantly flip the mass--ratio of the system, and a strong natal kick is needed for a successful CE phase and form a GW source.
    \item Utilizing the fact that there exist two highly asymmetric Galactic HMXBs, we estimate a rate of highly asymmetric GW190814--like events, which is within an order of magnitude of the observed rates for such events.
\end{itemize}

As the LVK detectors rapidly increase their sample of detected events in future observing runs, it would enable a better characterization of highly asymmetric mass-ratio BBH or NSBH systems. It would allow us to explore correlations in their spin, mass, and redshift distributions, which could potentially distinguish between the different formation pathways proposed for such systems. Additionally, several ongoing or future EM surveys Gaia, LSST, Roman, eROSITA will rapidly increase the sample of massive binaries in various intermediate stages of evolution. These systems, which can be probed with a wealth of observational techniques, provide a local anchor to understand various uncertainties in the evolution of massive stars and compact objects in binaries, and help us better understand the origin of GW sources.
